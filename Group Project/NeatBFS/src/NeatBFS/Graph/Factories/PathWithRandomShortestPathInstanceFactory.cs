...
{
    public class PathWithRandomShortestPathInstanceFactory : IShortestPathInstanceFactory
    {
        ...
        public PathWithRandomShortestPathInstanceFactory(int vertices, int pathLength, bool symmetricInstances = true, int? seed = null)
        {
            Vertices = vertices;
        ...
        public IEnumerable<ShortestPathTaskInstance> GenerateInstances()
        {
            while (true)
            {
                ...
                {
                    var f = path[_random.Value.Next(path.Count)];
                    path.Add(vertex);
